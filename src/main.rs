...
fn reset_grid(width: usize, height: usize, default_possible: Vec<value::Value>) -> Grid {
...
fn propagate(grid: &mut Grid, x: usize, y: usize, max_iterations: usize) {
    if max_iterations < 1 {
        return;
    }
...
fn collapse(grid: &mut Grid, x: usize, y: usize, max_iterations: usize) {
...
fn choose_collapsable(grid: &Grid) -> Option<(usize, usize)> {
...
fn render(grid: &Grid) {
    ...
    for col in grid {
        for t in col {
            if t.possible.len() == 1 {
                print!("{}", t.possible[0].value);
            }
            else {
                print!("{}", t.possible.len());
            }
        }
        println!();
    }
